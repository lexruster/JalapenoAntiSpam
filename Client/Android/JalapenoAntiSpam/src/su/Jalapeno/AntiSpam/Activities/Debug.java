...
public class Debug extends JalapenoActivity {
	...
	private void Init() {
		...
		_smsService = new SmsService(contactsService, jalapenoHttpService,
				new UserValidateService(), new LocalSpamBaseService(
						RepositoryFactory.getRepository()), new RequestQueue(
	...
	private void SettingsTest() {
		...
		Toast.makeText(
				this,
				String.format("Old key %s, new key %s", old.toString(),
						set2.PublicKey.toString()), Toast.LENGTH_LONG).show();
	}
	...
	private void TestInSpam() {
		LocalSpamBaseService spamBase = new LocalSpamBaseService(
